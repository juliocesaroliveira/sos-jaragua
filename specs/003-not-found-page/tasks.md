...
**20 de 24 concluídas.** Todo o código está implementado e verificado por `npx tsc --noEmit`,
`npm run lint`, `npm test` (125 testes) e `npm run build`.

Verificado em navegador, com sessão `usuario`:

- URL desconhecida autenticada → 404 com shell e menu do perfil, botão para `/` (T014, parcial)
- URL desconhecida anônima → 404 **sem** shell, botão para `/login`, **zero vazamento** (T017)
- Status HTTP `404` real nos dois casos, confirmado por rede e por `curl` (T021)
- `/dashboard` como `usuario` → `/sem-permissao`, **não** a página 404 (T020, parcial)
- 360px sem rolagem horizontal (T022, parcial)

### As 4 pendentes e por quê

Todas dependem de **autenticar com outros perfis**, e eu não digito senha em formulário.

- **T019** (US3, `notFound()` em `/atividades/<id>`) — a rota exige perfil interno; a sessão
  disponível é `usuario`, que recebe `/sem-permissao` antes de chegar ao `notFound()`.
- **T020** — a linha de `usuario` foi verificada; faltam `voluntario`→`/estoque`,
  `coordenador`→`/relatorios`, `membro_defesa_civil`→`/convocacao`. O mecanismo é o mesmo
  (`proxy.ts` + `exigirRoles`) e está coberto pelos testes de `rotas.test.ts`.
- **T022** — 360px verificado; falta a conferência por teclado nas duas variantes.
- **T024** — o quickstart completo depende das anteriores.

Para concluir: entre com `coordenador1@teste.local` (senha em
`specs/002-role-based-app-shell/quickstart.md`) e siga os Níveis 4, 5 e 7.
